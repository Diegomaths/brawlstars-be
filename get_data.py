...
import datetime
import os
new_week_download = datetime.datetime.fromtimestamp(os.path.getmtime("data/brawler_list.json")) < datetime.datetime.now().replace(microsecond=0) +datetime.timedelta(days=-7)

brawlers_info = get_api_data("https://api.brawlapi.com/v1/brawlers")
x = brawlers_info["list"]
n_brawlers = len(x)
actual_brawlers = len(pd.read_json("data/brawler_list.json"))
...
if refresh_brawlers or new_week_download:
    ...
    write_json(brawlers_info, "data/brawler_list.json")
    ...
    write_json(gadgets_df, "data/gadgets_list.json")

    starpowers_df = starpowers_df.set_index("name")
    starpowers_df = starpowers_df.set_index("name")
    write_json(starpowers_df, "data/starpowers_list.json")
...
write_json(maps_info, "data/full_maps_list.json")
...
write_json(maps_info, "data/mods_maps_list.json")