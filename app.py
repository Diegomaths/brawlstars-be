...
def run_get_data():
    try:
        ...
        out= f"Errore durante l'esecuzione di get_data.py: {e.stderr.decode()}"
        logging.error(out)
        return(out)
...
def get_data():
    try:
        ...
        logging.info(json_response["message"])
...
def delete_row():
    ...
    try:
        ...
        logging.info(f"removed row {to_drop}")
...
def edit_row():
    ...
    try:
        ...
        user_file = os.path.join(base_dir, current_user, "brawl_data.json")
        df.T.to_json(user_file, indent=2)
        logging.info(f"modified row {to_edit}")
